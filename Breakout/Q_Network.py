...
class Q_Network:
    ...
    def __init__(self, sess, num_states, num_actions, seed_n, layer_size_Q1, layer_size_Q2, learning_rate_Q, batch_size):
    ...
    def create_critic_net(self, name = "", num_states=4, num_actions=1, N_HIDDEN_1=400, N_HIDDEN_2=300):
        ...
        """
        W1_c = tf.get_variable(name+"W1", shape=[num_states, N_HIDDEN_1],
                             initializer=tf.contrib.layers.xavier_initializer(seed = self.seed_n))
        B1_c = tf.get_variable(name+"B1", shape=[N_HIDDEN_1],
                             initializer=tf.contrib.layers.xavier_initializer(seed = self.seed_n))
        
        W2_c = tf.get_variable(name+"W2", shape=[N_HIDDEN_1, N_HIDDEN_2],
                             initializer=tf.contrib.layers.xavier_initializer(seed = self.seed_n))
        B2_c = tf.get_variable(name+"B2", shape=[N_HIDDEN_2],
                             initializer=tf.contrib.layers.xavier_initializer(seed = self.seed_n))
        
        W2_action_c = tf.get_variable(name+"W2_action", shape=[num_actions, N_HIDDEN_2],
                             initializer=tf.contrib.layers.xavier_initializer(seed = self.seed_n))
        
        W3_c = tf.get_variable(name+"W3", shape=[N_HIDDEN_2, 1],
                             initializer=tf.contrib.layers.xavier_initializer(seed = self.seed_n))
        B3_c = tf.get_variable(name+"B3", shape=[1],
                             initializer=tf.contrib.layers.xavier_initializer(seed = self.seed_n))
