...
class Action_Network:
    ...
    def save_network(self, episode, save_epi):
        if episode == save_epi*1:
            self.saver.save(self.session, "/tmp/model1_A.ckpt")
        elif episode == save_epi*2:
            self.saver.save(self.session, "/tmp/model2_A.ckpt")
        elif episode == save_epi*3:
            self.saver.save(self.session, "/tmp/model3_A.ckpt")
        elif episode == save_epi*4:
            self.saver.save(self.session, "/tmp/model4_A.ckpt")
        elif episode == save_epi*5:
            self.saver.save(self.session, "/tmp/model5_A.ckpt")
        elif episode == save_epi*6:
            self.saver.save(self.session, "/tmp/model6_A.ckpt")
        elif episode == save_epi*7:
            self.saver.save(self.session, "/tmp/model7_A.ckpt")
        elif episode == save_epi*8:
            self.saver.save(self.session, "/tmp/model8_A.ckpt")
        elif episode == save_epi*9:
            self.saver.save(self.session, "/tmp/model9_A.ckpt")
        elif episode == save_epi*10:
            self.saver.save(self.session, "/tmp/model10_A.ckpt")
    
    def load_network(self, saved_num):
        if saved_num == 1:
            self.saver.restore(self.session, "/tmp/model1_A.ckpt")
        elif saved_num == 2:
            self.saver.restore(self.session, "/tmp/model2_A.ckpt")
        elif saved_num == 3:
            self.saver.restore(self.session, "/tmp/model3_A.ckpt")
        elif saved_num == 4:
            self.saver.restore(self.session, "/tmp/model4_A.ckpt")
        elif saved_num == 5:
            self.saver.restore(self.session, "/tmp/model5_A.ckpt")
        elif saved_num == 6:
            self.saver.restore(self.session, "/tmp/model6_A.ckpt")
        elif saved_num == 7:
            self.saver.restore(self.session, "/tmp/model7_A.ckpt")
        elif saved_num == 8:
            self.saver.restore(self.session, "/tmp/model8_A.ckpt")
        elif saved_num == 9:
            self.saver.restore(self.session, "/tmp/model9_A.ckpt")
        elif saved_num == 10:
            self.saver.restore(self.session, "/tmp/model10_A.ckpt")
    """    